<?php
/**
 * Comprehensive debug script for initial payment issues
 */
...
echo "<h2>Initial Payment Debug</h2>";
...
try {
    ...
    $bookingStmt = $conn->prepare("
        SELECT monthly_rent, security_deposit, start_date, end_date, status, payment_status
    ...
    if ($booking) {
        ...
        // Get monthly payments BEFORE test
        echo "<h4>Monthly Payments BEFORE Test:</h4>";
        $monthlyPayments = $tracker->getMonthlyPayments($testBookingId);
        
        echo "<table border='1' style='border-collapse: collapse;'>";
        echo "<tr><th>Month</th><th>Amount</th><th>Status</th><th>Payment Date</th><th>Method</th><th>Transaction ID</th></tr>";
        ...
        foreach ($monthlyPayments as $payment) {
            ...
            echo "<td>" . (isset($payment['transaction_id']) ? $payment['transaction_id'] : '-') . "</td>";
            echo "</tr>";
        ...
        if (isset($_GET['test_initial']) && !$hasAnyPaidPayments) {
            ...
            // Get next unpaid month before allocation
            $nextUnpaidBefore = $tracker->getNextUnpaidMonth($testBookingId);
            echo "<p><strong>Next unpaid month BEFORE allocation:</strong> " . ($nextUnpaidBefore ? date('F Y', strtotime($nextUnpaidBefore)) : 'None') . "</p>";
            ...
            if ($result['success']) {
                ...
                // Get monthly payments AFTER test
                ...
                // Get next unpaid month after allocation
                $nextUnpaidAfter = $tracker->getNextUnpaidMonth($testBookingId);
                echo "<p><strong>Next unpaid month AFTER allocation:</strong> " . ($nextUnpaidAfter ? date('F Y', strtotime($nextUnpaidAfter)) : 'None') . "</p>";
        ...
        $bookingPaymentsResult = $conn->query("
            SELECT id, booking_id, amount, payment_method, transaction_id, payment_date, status, notes
        ...
        if ($bookingPaymentsResult && $bookingPaymentsResult->num_rows > 0) {
            echo "<table border='1' style='border-collapse: collapse;'>";
            echo "<tr><th>ID</th><th>Amount</th><th>Method</th><th>Transaction ID</th><th>Payment Date</th><th>Status</th><th>Notes</th></tr>";
            
            while ($row = $bookingPaymentsResult->fetch_assoc()) {
                echo "<tr>";
                echo "<td>" . $row['id'] . "</td>";
                echo "<td>KSh " . number_format($row['amount'], 2) . "</td>";
            ...
            echo "<p>No booking payments found for this booking.</p>";
        }
